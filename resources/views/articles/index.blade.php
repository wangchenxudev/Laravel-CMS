      ...
      <div class="border-b border-slate-200 bg-slate-50/50 p-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-3 flex-1 max-w-2xl">
          ...
          <div class="w-full sm:w-44">
            <select class="block w-full px-3 py-1.5 text-sm rounded border border-slate-300 bg-white focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 focus:outline-none transition-all">
            ...
            @forelse ($articles as $article)
              <tr class="hover:bg-slate-50/50 transition-colors">
                ...
                <td class="px-6 py-4 font-semibold text-slate-900 max-w-xs truncate">
                  <a href="{{ route('articles.show', $article) }}" class="hover:text-brand-500 transition-colors">
                    {{ $article->title }}
                  </a>
                </td>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="inline-flex items-center rounded border px-2 py-0.5 text-xs font-semibold {{ $article->status->badgeClasses() }}">
                ...
                <td class="px-6 py-4 text-right whitespace-nowrap text-xs font-medium space-x-2">
                  <a href="{{ route('articles.show', $article) }}" class="text-brand-500 hover:text-brand-600 transition-colors">View</a>
                  @can('update', $article)
                    <a href="{{ route('articles.edit', $article) }}" class="text-slate-600 hover:text-slate-900 transition-colors">Edit</a>
                  @endcan
                  @can('delete', $article)
                    <form method="POST" action="{{ route('articles.destroy', $article) }}" class="inline" onsubmit="return confirm('Delete this article?');">
                      @csrf
                      @method('DELETE')
                      <button type="submit" class="text-rose-600 hover:text-rose-700 transition-colors">Delete</button>
                    </form>
                  @endcan
                </td>
              </tr>
